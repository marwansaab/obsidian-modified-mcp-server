...
**Scale/Scope**: Two new tools (`get_heading_contents`, `get_frontmatter_field`). One new module folder (`src/tools/surgical-reads/`) with four source files (`schema.ts`, `tool.ts`, `handler-heading.ts`, `handler-frontmatter.ts`). Two new methods on `ObsidianRestService`. Twenty-nine named contract tests across four files: 8 in `heading-handler.test.ts`, 11 in `frontmatter-handler.test.ts`, 8 in `schema.test.ts`, 2 in `registration.test.ts` (per [tasks.md T022](./tasks.md)), plus the small zod-error-propagation cases each suite adds. Net new TypeScript: ≤ ~500 lines including tests.
