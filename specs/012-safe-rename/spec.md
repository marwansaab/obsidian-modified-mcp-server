...
- **Source file does not exist**: `old_path` points to a path the vault does not contain. The tool propagates the upstream error from Obsidian (per FR-007) and must not create any file at `new_path`. Error message text comes from Obsidian, not the tool.
...
- **Cross-folder rename**: `new_path` is in a different folder than `old_path`, and the destination folder does not yet exist. The tool relies on Obsidian's built-in command behaviour for folder handling; if Obsidian's command fails because the parent folder is missing, the tool surfaces that failure rather than silently creating folders behind Obsidian's back (see FR-012 for the canonical "no auto-create / no `create_parents` flag" rule).
- **Path outside the vault**: Either path resolves outside the focused vault. The call results in an error from upstream (per FR-010 / FR-008); the underlying REST endpoints' path resolution detects the escape and the tool propagates the failure verbatim. The tool itself does not add a `..`/absolute-path filter.
- **Path resolves to a folder**: `old_path` identifies a folder (not a file). The call results in an error from upstream (per FR-001a / FR-008), with the error message coming from Obsidian — typically because `openFile` cannot open a folder as an editor. The tool must not attempt to dispatch Obsidian's "Rename folder" command as a fallback. Folder renaming will be handled by a separately-named tool in a future feature, with its own link-rewriting semantics.
- **Embed link integrity (attachments)**: `old_path` is a non-markdown attachment (e.g. `images/cover.png`) referenced by `![[cover.png]]` embed links elsewhere in the vault. With the required Obsidian setting enabled, embed link integrity is inherited from the same "Rename file" command — the tool adds no extra logic for the attachment case.
...
- **FR-001a**: When `old_path` resolves to a folder rather than a file, `rename_file` MUST result in an error (the underlying Obsidian command's failure response from `openFile` or `executeCommand` is propagated verbatim per FR-008) and MUST NOT modify any file or folder. The tool MUST NOT perform a pre-flight folder-vs-file check of its own; rejection of folder paths is delegated to Obsidian. (Error message text comes from Obsidian, not the tool — callers should not pattern-match on the message; folder-renaming is the responsibility of a future, separately-named `rename_folder` tool.)
- **FR-002**: `rename_file` MUST perform the rename by dispatching Obsidian's built-in "Rename file" command via the existing service-layer wrapper around `POST /commands/{commandId}/` (specifically the `ObsidianRestService.executeCommand` method, **not** the legacy MCP `execute_command` tool wrapper — that wrapper currently swallows per-command errors into success text, which would silently break FR-008 and Obsidian-level error propagation). The tool MUST NOT perform a filesystem-level rename, copy, or write of its own.
...
- **FR-004**: When the rename succeeds, every `[[wikilink]]` and `![[embed]]` reference in the vault that resolved to the old path MUST be updated to reference the new path, by virtue of Obsidian's "Automatically update internal links" behaviour being triggered (provided the user-controlled precondition in FR-005 holds). Markdown notes are typically referenced by `[[wikilinks]]`; attachments (e.g. `images/cover.png`) are typically referenced by `![[cover.png]]` embed links — both classes are inherited from the same Obsidian command and the tool adds no additional logic for either.
- **FR-004a**: When an updated wikilink originally carried a user-supplied alias (`[[target|alias]]`), the alias MUST be preserved in the rewritten reference. This behaviour is inherited from Obsidian's "Automatically update internal links" handling; the tool adds no logic of its own. Verification surface: User Story 1, Acceptance Scenario 2.
...
- **FR-006**: When `new_path` already identifies an existing file in the vault, `rename_file` MUST return an error (the underlying Obsidian command's failure response, propagated verbatim) and MUST NOT modify any file content or links. The tool MUST NOT perform a pre-flight collision check of its own; detection of the collision is delegated to Obsidian's command.
- **FR-007**: When `old_path` does not identify an existing file in the vault, `rename_file` MUST return an error (the underlying Obsidian command's failure response, propagated verbatim) and MUST NOT create a file at `new_path`. The tool MUST NOT perform a pre-flight existence check of its own; detection of the missing source is delegated to Obsidian's command.
