...
- **Path resolves to a folder**: `old_path` identifies a folder (not a file). The tool must reject the call with an error that names the rejected path and notes that folder renames are out of scope; folder renaming will be handled by a separately-named tool in a future feature, with its own link-rewriting semantics. The tool must not attempt to dispatch Obsidian's "Rename folder" command as a fallback.
...
- **FR-001a**: `rename_file` MUST reject any invocation where `old_path` resolves to a folder rather than a file, returning an error that names the rejected path and indicates that folder renames are out of scope for this tool. No file or folder may be modified in this case.
