...
- [ ] T006 [P] [US1] Update [tests/tools/delete-file/registration.test.ts](tests/tools/delete-file/registration.test.ts) per FR-012: extend the description-text assertion to require the schema string contains BOTH "When the path refers to a directory, the deletion is recursive" (spec 005 FR-011) AND "single direct-path verification query" (spec 007 FR-012), per [quickstart.md](specs/007-fix-delete-verify-direct/quickstart.md) § 3
...
- [ ] T021 Run `npm run lint`, `npm run typecheck`, and `npm run build` from the repository root; all three must succeed per Constitution § Development Workflow & Quality Gates 1–3 (the project's tsup build pipeline)
