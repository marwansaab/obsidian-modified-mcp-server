...
- [ ] T001 Install runtime dependency `zod-to-json-schema` by running `npm install --save-exact zod-to-json-schema` from the repo root; verify it appears under `dependencies` in `package.json`.
- [ ] T002 Install dev dependencies `vitest` and `nock` by running `npm install --save-dev --save-exact vitest nock` from the repo root; verify both appear under `devDependencies` in `package.json`.
- [ ] T003 Add `"test": "vitest run"` and `"test:watch": "vitest"` to the `scripts` block in `package.json`; run `npm test` and confirm it exits successfully (it will report "no test files found" — that is expected at this point).
...
- [ ] T004 Create the new feature directories: `src/tools/patch-content/` and `tests/tools/patch-content/`.
- [ ] T005 Create `src/tools/patch-content/schema.ts` exporting (a) `PatchRequestSchema` — a `zod` object schema for `{ filepath, operation, targetType, target, content, vaultId? }` matching the field types and enums defined in [data-model.md](./data-model.md#entity-patchrequest) and [contracts/patch_content.md §2](./contracts/patch_content.md#2-input-schema-equivalent-json-schema); and (b) `isValidHeadingPath(target: string): boolean` implementing the predicate in [data-model.md](./data-model.md#value-object-headingpath-validation-rule) (split on `::`, require ≥ 2 segments, every segment length ≥ 1, no trim, no escape).
- [ ] T006 Add to `src/tools/patch-content/schema.ts` an `assertValidPatchRequest(args: unknown): PatchRequest` function that (1) parses through `PatchRequestSchema`, (2) when `targetType === "heading"`, runs `isValidHeadingPath` and on failure throws an `Error` whose `message` contains all three required substrings per [data-model.md](./data-model.md#entity-wrappervalidationerror): the rule name (`heading targets must use the full H1::H2[::H3...] path`), `received: "<offending value>"`, and `e.g., "<corrected example>"`, and (3) returns the parsed value typed as `PatchRequest`.
...
- [ ] T007 [US2] Create `tests/tools/patch-content/schema.test.ts` covering contract test rows C2–C7 from [contracts/patch_content.md §7](./contracts/patch_content.md#7-test-matrix-contract-level): bare heading (`"Action Items"`), trailing-empty (`"A::B::"`), leading-empty (`"::A::B"`), middle-empty (`"A::::B"`), empty string (`""`), and whitespace-only (`"   "`). Each test calls `assertValidPatchRequest` from [src/tools/patch-content/schema.ts](../../src/tools/patch-content/schema.ts), expects it to throw, and asserts the thrown error's `message` contains: (a) the substring `H1::H2`, (b) `received: "<that-test's-input>"`, and (c) `e.g.,`. Run `npm test` and confirm all six rejection cases pass.
...
- [ ] T008 [P] [US1] Create `src/tools/patch-content/tool.ts` exporting `PATCH_CONTENT_TOOLS: Tool[]` (single entry). Build the entry with `name: 'patch_content'`, an `inputSchema` produced at module-load time by `zodToJsonSchema(PatchRequestSchema, { name: 'PatchRequest' })` (or equivalent — match the JSON Schema shape in [contracts/patch_content.md §2](./contracts/patch_content.md#2-input-schema-equivalent-json-schema)), and a `description` string that contains all three required substrings per [contracts/patch_content.md §1](./contracts/patch_content.md#1-tool-registration): the full-path requirement (`H1::H2`), the top-level-unreachable note (`top-level`), and the literal-`::` unreachable note (`::`). Exact prose is at the implementer's discretion.
- [ ] T009 [P] [US1] Create `src/tools/patch-content/handler.ts` exporting `async function handlePatchContent(args: Record<string, unknown>, rest: ObsidianRestService): Promise<CallToolResult>`. Implementation: (1) `const req = assertValidPatchRequest(args);` (2) `await rest.patchContent(req.filepath, req.operation, req.targetType, req.target, req.content);` (3) `return { content: [{ type: 'text', text: 'Content patched successfully' }] };`. Do **NOT** wrap step 2 in `try/catch` — upstream errors must propagate per Constitution Principle IV.
- [ ] T010 [US1] Modify `src/tools/write-tools.ts`: import `PATCH_CONTENT_TOOLS` from `./patch-content/tool.js`; spread it into the `WRITE_TOOLS` array; **delete the entire `// DISABLED: patch_content...` commented block** (lines 52-86) including the workaround comment.
- [ ] T011 [P] [US1] Modify `src/index.ts`: add `import { handlePatchContent } from './tools/patch-content/handler.js';` to the imports; in the `handleToolCall` switch, **delete the entire `// DISABLED: patch_content handler...` commented block** (lines 333-351) and replace it with an active case: `case 'patch_content': return handlePatchContent(args, rest);`. (Parallel with T010 — different files, no mutual dependency.)
- [ ] T012 [P] [US1] Create `tests/tools/patch-content/handler.test.ts` covering contract test rows C1, C8, C9 from [contracts/patch_content.md §7](./contracts/patch_content.md#7-test-matrix-contract-level). Use the test harness pattern in [quickstart.md §4](./quickstart.md#4-write-the-tests): construct an `ObsidianRestService` pointing at `https://localhost:27123`, set up `nock('https://localhost:27123')` in `beforeEach`, `nock.cleanAll()` in `afterEach`. C1: heading patch with `target: 'Weekly Review::Action Items'`, `nock` returns 200, expect `result.isError` to be falsy and the response text to be `Content patched successfully`, expect `scope.isDone()` to be `true`. C8: `targetType: 'block'`, `target: 'whatever'`, `nock` returns 200, expect success and the upstream call to have been made (no validator path). C9: `targetType: 'frontmatter'`, same shape as C8.
- [ ] T013 [P] [US1] Create `tests/tools/patch-content/registration.test.ts` covering contract test row C12. Import `ALL_TOOLS` from `src/tools/index.ts`. Locate the entry whose `name === 'patch_content'`; assert that `entry.description` contains, case-insensitively, all three required substrings: `H1::H2`, `top-level`, and `::`. (Depends on T010 wiring `PATCH_CONTENT_TOOLS` into `WRITE_TOOLS`.)
...
- [ ] T014 [US3] Add to `tests/tools/patch-content/handler.test.ts` the contract test rows C10 and C11 from [contracts/patch_content.md §7](./contracts/patch_content.md#7-test-matrix-contract-level). C10: valid request, `nock` returns `404` with body `{ errorCode: 40400, message: "File or heading not found" }`; expect the call to throw (or for `handlePatchContent` to reject) such that the eventual MCP response — formed by the existing top-level handler in `src/index.ts:250-257` — would contain text matching `/Obsidian API Error 40400.*File or heading not found/`. C11: valid request, `nock.disableNetConnect()` plus a deliberately unrouted scope so the call surfaces as a transport error; expect the rejection's message to contain `Obsidian API Error -1` and the underlying network error text. The handler **must not** catch these — verify by asserting `handlePatchContent` itself rejects (rather than returning a structured success).
...
- [ ] T015 [P] Run `npm run lint` from the repo root; resolve any reported issues so it exits with code 0.
- [ ] T016 [P] Run `npm run typecheck` from the repo root; resolve any type errors so it exits with code 0.
- [ ] T017 [P] Run `npm run build` from the repo root; verify `dist/` is produced without errors.
- [ ] T018 Run `npm test` from the repo root; verify all 12 contract tests (C1–C12) pass.
- [ ] T019 Smoke-test against a real Obsidian instance per [quickstart.md §7](./quickstart.md#7-smoke-test-against-a-real-obsidian-optional). Confirm both: (a) a valid 2-segment heading patch lands in the correct subsection, and (b) a bare-target call returns a validation error and the Local REST API plugin's request log shows **no** PATCH request was made.
- [ ] T020 Author the PR description per the Constitution Governance section: include a one-line statement confirming Principles I–IV were considered, link to [plan.md](./plan.md), and call out the new dev-tooling additions (vitest, nock, zod-to-json-schema) as the first test infrastructure in the repo.
