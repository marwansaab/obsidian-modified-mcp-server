...
- Q: Does the tool process `.md` files inside dot-prefixed directories (`.obsidian/`, `.trash/`, etc.)? → A: Exclude all dot-prefixed directories by default; no override in this round. Any path segment starting with `.` excludes the file from enumeration; such files are not counted toward `filesScanned`. Protects `.obsidian/` plugin configs and similar from accidental mutation. Override (e.g., `includeHidden`) is a future extension.
- Q: How does the tool detect HTML comment boundaries? → A: Non-greedy `<!--…-->` spanning newlines, no nesting. The skip region is the smallest span where `<!--` is followed by `-->` (region-detector uses dotall internally regardless of the user's search-regex flags — region detection is independent of FR-013's flag set). The first `-->` closes the comment per HTML spec. Empty comments (`<!---->`, `<!-- -->`) are honored. Unclosed `<!--` runs to end-of-file, mirroring FR-007's unclosed-fence handling.
- Q: How does the tool handle a regex that matches a zero-width / empty position (e.g., `/^/gm`, `/(?=x)/g`)? → A: Allow empty-match regexes; rely on JavaScript's standard auto-advance semantics; each empty match counts as one replacement. To prevent unbounded output growth, the tool caps post-replacement file size at the same 5 MB threshold as input (FR-024a) — files whose rewritten size would exceed the cap are skipped and reported in `skipped` with reason `output_size_exceeded`.
- Q: Is the `.md` extension match case-sensitive or case-insensitive? → A: Case-insensitive. Files ending in `.md`, `.MD`, `.Md`, `.mD` are all processed. Matches Obsidian's own behavior and Windows filesystem semantics. Note: this is independent of the `pathPrefix` rule (Q5 / session 2), which remains case-sensitive on all platforms — extension matching is a filesystem-layer concern, while `pathPrefix` is a user-intent concern.
- Q: How is `totalReplacements` defined? → A: Sum of actual replacements across all modified files (matches actually performed; skipped-region matches and skipped-file matches NOT counted; dry-run counts what would be replaced). Plus an additional `totalMatchesInSkippedRegions` count in the response for transparency, so users can validate that skipCodeBlocks / skipHtmlComments are excluding what they expect.
- Q: What format does the dry-run preview use for the abridged before/after? → A: Structured per-match objects. Each match preview is `{ matchIndex (1-based), lineNumber (1-based), columnStart (1-based, char index), before (≤40 chars left context), match (matched text), replacement (what would replace it), after (≤40 chars right context) }`. Testable, language-agnostic, bounded response size; clients can render any way they choose.
- Q: How are the result arrays (`perFile`, `failures`, `skipped`) ordered in the response? → A: Sort each array by vault-relative path, ascending, lexicographic over UTF-8 code points. All three arrays use the same canonical order, regardless of underlying filesystem-walk or REST API enumeration order. Within `perFile[i].previews`, match objects are ordered by `matchIndex` (scan order). Deterministic across platforms and runs.
- Q: When `wholeWord` / `flexibleWhitespace` and `skipCodeBlocks` / `skipHtmlComments` compose, what's the sequencing? Can a single match cross a skip-region boundary? → A: Skip regions are detected first and carved out; the pattern is then evaluated only against the concatenated searchable spans. `wholeWord` / `flexibleWhitespace` operate on those spans. A single match MUST NOT cross a skip-region boundary (cannot start in prose and end inside a code block). `\b` boundaries at the edges of skipped regions are computed against the literal characters at those edges in the original content, preserving predictable word-boundary semantics near fences and comments.
...
1. **Given** a note with the search string in prose, in a fenced code block, and in an HTML comment, **When** the user calls `find_and_replace` with `skipCodeBlocks: true` and `skipHtmlComments: true`, **Then** the prose match is replaced; the code-block and HTML-comment bytes are byte-identical to the input; the response reports `totalReplacements = 1` and `totalMatchesInSkippedRegions = 2` (one each for code-block and comment).
2. **Given** the same note, **When** the user calls without those flags (defaults `false`), **Then** all three matches are replaced; `totalReplacements = 3` and `totalMatchesInSkippedRegions = 0`.
...
- **Per-file size cap (input and output)**: Files whose fetched content exceeds 5 MB are skipped with reason `size_exceeded`. Files whose post-replacement output would exceed 5 MB (a real risk with empty-match regexes like `/^/gm` paired with a long replacement) are NOT written and are skipped with reason `output_size_exceeded`; both kinds appear in the response's `skipped` array. The sweep continues with remaining files. Skipped files differ from `failures` — `skipped` means preventatively-not-attempted-or-not-written; `failures` means attempted-and-errored. Users who need to process larger files SHOULD split the file or wait for a future configurable-cap extension.
...
- **FR-008**: When `skipHtmlComments: true`, the tool MUST exclude content inside HTML comments from the search; the bytes inside skipped regions MUST be preserved byte-for-byte. Comment boundaries MUST be detected using non-greedy `<!--…-->` spanning newlines — the skip region is the smallest span where `<!--` is followed by `-->`. The region detector MUST treat `.` as matching any character including newlines for the purpose of region detection ONLY (independent of FR-013's outer regex flag set, which governs the user's `search` pattern). The first `-->` closes the comment per HTML spec; subsequent `-->` characters inside the same comment are not honored as nested closers (HTML does not support nested comments). Empty comments (`<!---->`, `<!-- -->`) MUST be honored as zero-or-near-zero-byte skip regions. If a `<!--` is unclosed (no matching `-->` to end-of-file), the tool MUST treat everything from the opener to end-of-file as inside the comment, mirroring FR-007's unclosed-fence handling.
...
- **FR-009a**: Sequencing of skip-region detection vs. pattern modifiers: skip regions MUST be detected and carved out *first*; the pattern (after `wholeWord` / `flexibleWhitespace` rewriting) MUST then be evaluated only against the concatenated searchable spans (i.e., the byte ranges that are NOT in a skipped region). A single match MUST NOT span a skip-region boundary — the tool MUST NOT produce a match that starts in a searchable span and ends inside a skipped region (or vice versa). At the boundaries of skipped regions, `\b` word-boundary checks MUST be evaluated against the literal characters at those edges in the *original* content, preserving predictable word-boundary semantics near fences and comments.
...
- **FR-015**: When `dryRun: true`, the tool MUST perform zero writes (no PUT, no mtime changes) and MUST return per-file diff previews suitable for human review. Each per-file preview MUST include `filename` (string, vault-relative path), `replacements` (count of matches that would be replaced), and `previews` (array of structured match objects, capped at the first 3 matches per file by default; the cap is a fixed default in this round and is not user-configurable). Each match-preview object MUST have the shape: `{ matchIndex (1-based, integer), lineNumber (1-based, integer), columnStart (1-based, character index, integer), before (string, ≤40 characters of left context), match (string, the matched text), replacement (string, what would replace the match), after (string, ≤40 characters of right context) }`. Context strings (`before`, `after`) MUST be truncated at the 40-char boundary at the same character cost regardless of how many bytes the characters consume (truncation is by Unicode code points, not bytes). Newlines inside context MUST be preserved literally (so the user can see line context).
...
- **FR-020**: On success, the tool MUST return a structured response containing at minimum: `ok: true`, `filesScanned` (count), `filesModified` (count), `totalReplacements` (count, defined per FR-020a), `totalMatchesInSkippedRegions` (count, defined per FR-020b), and `filesSkipped` (count; zero when no files were skipped by the size cap). When `verbose: true`, the response MUST also contain `perFile` (array of `{ filename, replacements, ... }` entries). The `skipped` array MUST be present in the response whenever any file was skipped by the size cap (per FR-024a), regardless of `verbose`. On partial success (per FR-021a), the same response shape applies with `ok: false` plus a non-empty `failures` array.
- **FR-020a**: `totalReplacements` MUST be defined as the sum of *actual replacement operations performed* across all modified files. (a) Empty/zero-width matches count as one replacement each (consistent with FR-013's empty-match handling). (b) Matches that fall inside skipped regions (per FR-007 / FR-008 / FR-009) MUST NOT be counted toward `totalReplacements`. (c) Matches in files that hit the input size cap, output size cap, or were excluded by FR-024b dot-prefix rule MUST NOT be counted. (d) For dry-run (FR-015), `totalReplacements` MUST count what *would* be replaced under the same rules.
- **FR-020c**: The result arrays in the response — `perFile` (when `verbose: true`), `failures` (when present), and `skipped` (when present) — MUST each be ordered by vault-relative path, ascending, lexicographic over UTF-8 code points. The same canonical ordering MUST be used for all three arrays regardless of the underlying filesystem-walk order or REST API enumeration order. Within `perFile[i].previews`, match objects MUST be ordered by `matchIndex` (1-based, in scan order from the start of the file). This determinism is a precondition for test assertions on specific array positions.
- **FR-020b**: `totalMatchesInSkippedRegions` MUST be defined as the sum of matches in the *original content* that fell inside skipped regions (code blocks, HTML comments) and were therefore NOT replaced. The count is across all scanned (not skipped-file) files. This is a transparency aid so users can validate that `skipCodeBlocks` / `skipHtmlComments` excluded the matches they expected. When neither skip flag is set, `totalMatchesInSkippedRegions` MUST be zero.
...
- **FR-024**: The tool MUST NOT process non-`.md` files even if they appear under `pathPrefix`. The `.md` extension match MUST be case-insensitive — `.md`, `.MD`, `.Md`, and `.mD` all qualify as Markdown and MUST be processed. This is independent of FR-004's `pathPrefix` casing rule (which remains case-sensitive); extension matching is a filesystem-layer concern, while `pathPrefix` is a user-intent concern.
- **FR-024b**: The tool MUST exclude any file whose vault-relative path contains a dot-prefixed path segment (e.g., `.obsidian/foo.md`, `.trash/bar.md`, `subdir/.hidden/baz.md`). Excluded files MUST NOT be counted toward `filesScanned`, MUST NOT appear in the `skipped` array (skipped is reserved for size-cap exclusions), and MUST NOT be touched. This protects Obsidian's plugin configs and other vault-private state from accidental mutation. No override parameter is exposed in this round.
- **FR-024a**: The tool MUST enforce a per-file soft size cap of 5 MB (5,242,880 bytes) on **both input and output**. Files whose fetched content exceeds the cap MUST NOT be processed (no replacement, no write); such files MUST appear in the `skipped` array with `{ filename, reason: "size_exceeded", sizeBytes }`. Files whose post-replacement output would exceed the cap (a real risk with empty-match regexes inserting non-empty replacements per line, e.g., `/^/gm` with a long replacement) MUST also NOT be written; such files MUST appear in the `skipped` array with `{ filename, reason: "output_size_exceeded", inputSizeBytes, outputSizeBytes }`. The sweep MUST continue with remaining files. Skipped files (either reason) MUST NOT count toward `filesModified`. Skipped files are distinct from `failures` (skipped = preventatively-not-attempted-or-not-written; failures = attempted-and-errored).
