...
- Q: How should the regression-test FRs (FR-006, FR-013) behave under Path C, given they presume a dispatcher branch that does not exist? → A: Make them conditional on Paths A/B. Under Path C, both FRs do not apply. A new FR-014 mandates a contract test that queries the MCP server's advertised tool catalog (e.g. via the `tools/list` mechanism the MCP SDK exposes) and asserts that none of the seven graph tool names appears in the result. Runs on every CI build. Catches a future contributor who re-adds a schema without wiring the dispatcher branch — exactly the failure mode this fix addresses.
...
A future contributor changes either the dispatcher (under Paths A/B) or the advertised tool catalog (under Path C). Either way, a regression test fails immediately, surfacing the contract drift before it ships. This prevents the same class of bug — schema and dispatcher diverging — from recurring silently. The exact test that fires depends on the path chosen: under A/B the `get_vault_stats` deep test (FR-006) and the smoke test for the other six (FR-013); under C the contract test that asserts the seven names are absent from the advertised catalog (FR-014).
...
4. **Given** Path C was taken (no dispatcher branches for graph tools exist), **When** a future contributor re-adds any of the seven graph-tool schemas to the advertised catalog without wiring a corresponding dispatcher branch, **Then** the FR-014 contract test fails immediately, naming the re-added schema(s) and preventing the regression from shipping.
...
- **Path C chosen, then later reversed**: If a future contributor re-adds any of the seven schemas, the FR-014 contract test fails immediately (it asserts those names are absent from the advertised catalog). The contributor MUST then wire the corresponding dispatcher branches and replace FR-014 with the FR-006 deep test plus the FR-013 smoke test set. No "skipped/pending scaffold" is carried forward under Path C — FR-014 is the only graph-tool test that exists in that state, and it actively guards the absence.
...
- **FR-006**: Under Paths A and B, a regression test MUST cover `get_vault_stats` end-to-end through the dispatcher, mocking the underlying graph backend and asserting both that the wrapper invokes it correctly and that the mocked response is parsed into the tool result. Under Path C, this requirement does not apply — see FR-014.
...
- **FR-013**: Under Paths A and B, a parametrized smoke test MUST cover the six graph tools other than `get_vault_stats` (i.e. `get_vault_structure`, `find_orphan_notes`, `get_note_connections`, `find_path_between_notes`, `get_most_connected_notes`, `detect_note_clusters`). Each parameter row MUST call the tool with minimal valid inputs and assert the response is NOT an `Unknown tool: <name>` error. Other error responses (missing arguments, vault path not configured, etc.) are acceptable — they prove the dispatcher routed the call to a handler, which is what this test guards against. When a tool is added to or removed from the graph set, the parameter list updates; the assertion logic stays the same. Under Path C, this requirement does not apply — see FR-014.
- **FR-014**: Under Path C, a contract test MUST query the MCP server's advertised tool catalog (e.g. via the `tools/list` mechanism the MCP SDK exposes) and assert that none of the seven graph tool names — `get_vault_stats`, `get_vault_structure`, `find_orphan_notes`, `get_note_connections`, `find_path_between_notes`, `get_most_connected_notes`, `detect_note_clusters` — appears in the result. The test MUST run on every CI build. If a future contributor re-adds any of the seven schemas without wiring the corresponding dispatcher branch, FR-014 fails before the regression ships — which is exactly the failure mode that motivated this fix.
...
- **SC-007**: Under Path C, the FR-014 contract test passes on a clean checkout and fails immediately if any of the seven graph-tool schemas is re-added to the advertised catalog, naming the re-added schema(s) in the failure message.
