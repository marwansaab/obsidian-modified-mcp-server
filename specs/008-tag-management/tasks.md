...
- [ ] T004 [P] [US1] Write `tests/tools/list-tags/handler.test.ts` covering the happy path: nock the upstream `GET /tags/` to return the documented body shape including a hierarchical example (`{ tags: [{ name: 'project', count: 3 }, { name: 'work/tasks', count: 5 }, { name: 'work', count: 5 }] }`); call `handleListTags({}, rest)` and assert the returned `CallToolResult.content[0].text` parses back to the exact same object (FR-012 verbatim pass-through). Pin the HTTP method (`GET`), URL path (`/tags/`), and `Authorization: Bearer <key>` header on the nock interceptor (FR-010)
